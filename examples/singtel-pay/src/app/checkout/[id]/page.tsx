...
type CheckoutData = {
  ucp?: {
    ...
    payment_handlers?: Record<string, Array<{ id: string; version: string; config?: unknown }>>;
  };
  id: string;
  status: string;
...
export default function CheckoutPage() {
  ...
  return (
    <div style={{ maxWidth: 520, margin: "40px auto", padding: 24 }}>
      ...
      <div style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 16, marginBottom: 16 }}>
        ...
        {checkout?.totals && (
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #e5e7eb" }}>
            {checkout.totals.map((total, idx) => (
              <div key={idx} style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ textTransform: "capitalize" }}>{total.type}:</span>
                <span style={{ fontWeight: total.type === "total" ? 700 : 400 }}>
                  {checkout.currency} {(total.amount / 100).toFixed(2)}
                </span>
              </div>
            ))}
          </div>
      ...
      {checkout?.messages && checkout.messages.length > 0 && status !== "completed" && (
        <div style={{
          padding: 12,
          background: "#fef3c7",
          ...
          {checkout.messages.map((msg, idx) => (
            <p key={idx} style={{ color: "#92400e", fontSize: 14, marginBottom: idx < checkout.messages!.length - 1 ? 6 : 0 }}>
              {msg.content}
            </p>
          ))}
        </div>
